...
function renderVennChart() {
	...
	const tooltip = select('body .wrapper-venn')
		.append('div')
	...
	vennDiv
		...
		.on('mouseover', function (d: any, i: number) {
			...
			tooltip
				...
				.style('opacity', props.tooltipOpacity)
		...
		.on('mousemove', function (d: any, i: number) {
			tooltip
				.style('left', event.pageX + props.tooltipOffsetX + 'px')
				.style('top', event.pageY + props.tooltipOffsetY + 'px')
		})
