(function(){"use strict";try{if(typeof document<"u"){var e=document.createElement("style");e.appendChild(document.createTextNode('.wrapper-venn[data-v-c4b3a31d]{width:100%;height:100%}.wrapper-venn[data-v-c4b3a31d] .custom-venn-chart-tooltip{position:absolute;min-height:32px;max-width:200px;font-size:16px;display:flex;align-items:center;text-align:left;padding:8px 16px 8px 36px;background:#fff;color:#606266;border:0;border-radius:8px;opacity:0;box-shadow:0 2px 8px #00000026;z-index:2333}.wrapper-venn[data-v-c4b3a31d] .custom-venn-chart-tooltip:before{content:"";width:10px;height:10px;position:absolute;left:16px;background-color:var(--45cb2ac0);border-radius:50%;top:50%;transform:translateY(-50%)}')),document.head.appendChild(e)}}catch(t){console.error("vite-plugin-css-injected-by-js",t)}})();
import { defineComponent as Bi, useCssVars as Lo, ref as $n, watch as Fo, onMounted as Oo, getCurrentInstance as zo, onUnmounted as Do, openBlock as qo, createElementBlock as Ho, pushScopeId as Xo, popScopeId as Bo, createElementVNode as Vo } from "vue";
function Vi(t, n) {
...
const ix = (t) => (Xo("data-v-c4b3a31d"), t = t(), Bo(), t), sx = { class: "wrapper-venn" }, ox = /* @__PURE__ */ ix(() => /* @__PURE__ */ Vo("div", { id: "venn" }, null, -1)), ax = [
  ox
], ux = Bi({
  name: "VennChart"
}), fx = /* @__PURE__ */ Bi({
  ...
  setup(t, { emit: n }) {
    ...
    function a() {
      ...
      f.datum(e.data).call(l), f.selectAll("g path").style("fill", function(h, d) {
        return e.colors[d];
      }).style("fill-opacity", 0.7).style("stroke-opacity", 0).style("stroke-width", e.strokeWidth).style("stroke", e.strokeColor), f.selectAll("text").style("fill", e.textColor).style("font-size", "20px").style("font-weight", "700");
      const c = Ft("body .wrapper-venn").append("div").attr("class", "custom-venn-chart-tooltip");
      f.selectAll("g").on("mouseover", function(h, d) {
        s.value = e.colors[d], c.transition().duration(e.tooltipDelay).style("opacity", e.tooltipOpacity), c.text(`${e.legend[d]} ${h.label}`), Ft(this).transition("tooltip").duration(e.tooltipDelay).select("path").style("stroke-opacity", 1).style("fill-opacity", 1).style("fill", e.colors[d]);
      }).on("mousemove", function(h, d) {
        c.style("left", Qt.pageX + e.tooltipOffsetX + "px").style("top", Qt.pageY + e.tooltipOffsetY + "px");
      }).on("mouseout", function(h, d) {
        c.transition().duration(e.tooltipDelay).style("opacity", 0), Ft(this).transition("tooltip").duration(e.tooltipDelay).select("path").style("stroke-opacity", 0).style("fill-opacity", 0.7).style("fill", e.colors[d]);
...
const lx = (t, n) => {
  ...
  for (const [r, i] of n)
    e[r] = i;
  return e;
}, Fe = /* @__PURE__ */ lx(fx, [["__scopeId", "data-v-c4b3a31d"]]);
Fe.install = (t) => {
